...
(function ($) {
    ...
    var methods = {
        ...
        copyItem: function (widgetOptions, e, $elem) {
            _copyItem(widgetOptions, e, $elem);
        },
    ...
    var _parseTemplate = function (widgetOptions) {
        ...
        //clear template from nested dynamicforms with min = 0
        $template.find('div[data-dynamicform]').each(function () {
            var formObject = eval($(this).attr('data-dynamicform'));
            if (formObject.min === 0) {
                $template.find(formObject.widgetItem).remove();
            }
        });
    ...
    var _copyItem = function (widgetOptions, e, $elem) {
        var count = _count($elem, widgetOptions);
        ...
        if (count < widgetOptions.limit) {
            ...
            $copyItem = $elem.closest(widgetOptions.widgetItem);

            // Distinct dynamic form items recursively
            __distinctRecursive('[data-dynamicform^=dynamicform]', $newclone);

            //copy nested dynamicforms
            __copyRecursive('[data-dynamicform^=dynamicform]', $newclone, $copyItem);
            //copy values
            __copyValues($newclone, $copyItem);

            $newclone.insertAfter($copyItem);

            _updateAttributes(widgetOptions);
            _restoreSpecialJs(widgetOptions);
            _fixFormValidaton(widgetOptions);
            $elem.closest('.' + widgetOptions.widgetContainer).triggerHandler(events.afterCopy, $newclone);
        } else {
            // trigger a custom event for hooking
            $elem.closest('.' + widgetOptions.widgetContainer).triggerHandler(events.limitReached, widgetOptions.limit);
        }
    ...
    // Copy nested dynamicform recursively
    var __copyRecursive = function (regex, $item, $copyItem) {
        var $items = $item.find(regex);
        $.each($items, function () {
            $form = $(this);
            var formObject = eval($form.data('dynamicform'));
            var $nestedCopyItem = $copyItem.find(formObject.widgetItem);
            var copyCount = $nestedCopyItem.length;
            for (var i = formObject.min; i < copyCount; i++) {
                $nestedtoclone = formObject.template;
                $nestednewclone = $nestedtoclone.clone(false, false);
                // __distinctRecursive('[data-dynamicform^=dynamicform]', $nestednewclone);
                $form.find(formObject.widgetBody).append($nestednewclone);
                // _updateAttributes(formObject);
                // _restoreSpecialJs(formObject);
                // _fixFormValidaton(formObject);
                $form.triggerHandler(events.afterInsert, $nestednewclone);
            }

            __copyRecursive(regex, $form, $nestedCopyItem)
        });
    };

    var __copyValues = function ($item, $copyItem) {
        $copyFields = $copyItem.find('input, textarea, select');
        $item.find('input, textarea, select').each(function (index) {
            // console.log($(this).attr('id'), $copyFields.eq(index).attr('id')); //debug wrong copy especially when nested forms
            if ($(this).is(':checkbox') || $(this).is(':radio')) {
                $(this).prop('checked', $copyFields.eq(index).prop('checked'));
            } else if ($(this).is('select')) {
                $selectedOption = $copyFields.eq(index).find('option:selected');
                if ($selectedOption.length) {
                    $(this).find('option[value="' + $selectedOption.val() + '"]').attr("selected", "selected");
                }
            } else if ($(this).is('[name*="[id]"]:hidden')) {
                $(this).val('');
            } else {
                $(this).val($copyFields.eq(index).val());
            }
        });
    };
    ...
    var _restoreSpecialJs = function (widgetOptions) {
        ...
        if ($hasSpectrum.length > 0) {
            $hasSpectrum.each(function () {
                ...
                jQuery(matches[1] + matches[2] + "color").on('change', function () {
                    jQuery(id).val(this.value)
                });
                jQuery(id).on('input change', function (e) {
                    jQuery(matches[1] + matches[2] + "color").val(this.value);
                    if (e.type == 'change') {
                        jQuery(matches[1] + matches[2] + "color").trigger('change');
                    }
                });
        ...
        //collapse reindex
        $collapse = $(widgetOptionsRoot.widgetItem).find('.collapse');
        if ($collapse.length > 0) {
            $collapse.each(function () {
                var id = $(this).attr('id');
                $(this).closest(widgetOptionsRoot.widgetItem).find('[data-toggle="collapse"]').attr('data-target', '#'+id);
            });
        }
    };
